...
class ExportManager {
    ...
    private func createZipArchive(sourceURL: URL, destinationURL: URL) throws {
        ...
        // Change to the source directory and zip its contents to temp location
        let zipCommand = """
        cd "\(sourceURL.path)" && /usr/bin/zip -r -X "\(tempZip.path)" .
        """

        process.arguments = ["-c", zipCommand]
    ...
    // Word-optimized HTML with inline styles (better for DOCX export)
    private func markdownToWordHTML(_ markdown: String) -> String {
        var html = """
        <!DOCTYPE html>
        <html>
        <head>
        ...
        while i < lines.count {
            ...
            if line.hasPrefix("```") {
                if inCodeBlock {
                    ...
                    html += "<pre style=\"background-color: #f5f5f5; padding: 10pt; margin: 10pt 0; border: 1px solid #ddd; font-family: 'Courier New', Courier, monospace; font-size: 9pt;\">"
                    inCodeBlock = true
                }
                i += 1
                continue
            ...
            // Tables - convert to simpler format for better Word compatibility
            if line.hasPrefix("|") && line.hasSuffix("|") {
                if inList {
                    html += "</ul>\n"
                    inList = false
                ...
                html += "<table border=\"1\" cellpadding=\"8\" cellspacing=\"0\" style=\"border-collapse: collapse; font-size: 10pt;\">\n"
                var isFirstRow = true
                ...
                while i < lines.count && lines[i].hasPrefix("|") {
                    ...
                    if !cells.isEmpty {
                        ...
                        for cell in cells {
                            if isFirstRow {
                                html += "<th bgcolor=\"#E8E8E8\"><b>\(formatInlineMarkdown(cell))</b></th>"
                            } else {
            ...
            if line.hasPrefix("#### ") {
                if inList { html += "</ul>\n"; inList = false }
                html += "<h4 style=\"font-size: 12pt; font-weight: 600; margin-top: 10pt; margin-bottom: 4pt;\">\(formatInlineMarkdown(String(line.dropFirst(5))))</h4>\n"
            } else if line.hasPrefix("### ") {
                if inList { html += "</ul>\n"; inList = false }
                html += "<h3 style=\"font-size: 14pt; font-weight: 600; margin-top: 12pt; margin-bottom: 6pt;\">\(formatInlineMarkdown(String(line.dropFirst(4))))</h3>\n"
            } else if line.hasPrefix("## ") {
                if inList { html += "</ul>\n"; inList = false }
                html += "<h2 style=\"font-size: 18pt; font-weight: 600; margin-top: 16pt; margin-bottom: 8pt;\">\(formatInlineMarkdown(String(line.dropFirst(3))))</h2>\n"
            } else if line.hasPrefix("# ") {
                if inList { html += "</ul>\n"; inList = false }
                html += "<h1 style=\"font-size: 24pt; font-weight: 600; margin-top: 18pt; margin-bottom: 12pt; padding-bottom: 8pt; border-bottom: 2px solid #333;\">\(formatInlineMarkdown(String(line.dropFirst(2))))</h1>\n"
            }
            // Lists
            else if line.hasPrefix("- ") || line.hasPrefix("* ") || line.hasPrefix("+ ") {
                if !inList {
                    html += "<ul style=\"margin: 10pt 0; padding-left: 24pt;\">\n"
                    inList = true
                }
                let itemText = String(line.dropFirst(2))
                html += "<li style=\"margin: 4pt 0;\">\(formatInlineMarkdown(itemText))</li>\n"
            }
            // Empty line
            else if line.trimmingCharacters(in: .whitespaces).isEmpty {
                if inList {
            ...
            // Regular paragraph
                ...
                html += "<p style=\"margin-top: 0; margin-bottom: 10pt;\">\(formatInlineMarkdown(line))</p>\n"
            }
